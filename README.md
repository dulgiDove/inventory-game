...
## 개발 배경
이전 프로젝트에서 기능을 하나씩 덧붙이는 방식으로 개발하다 코드가 얽혀 유지 보수가 어려워지는 문제를 경험했습니다. 

이를 해결하기 위해 Component Pattern을 처음부터 적용한 프로젝트를 만들어보고자 생각이 들어 시작했고, 그 결과물이 이 프로젝트입니다. 

아이템의 효과, 착용 조건, 장비 스탯 등을 독립적인 컴포넌트로 분리하여 기존 코드 수정 없이 조합만으로 새로운 아이템을 추가할 수 있도록 설계했습니다.

## 스크린샷(GIF)

player가 움직이고, Crafter와 상호작용하고, 아이템을 제작하고, 인벤토리에서 툴팁 확인하는 게임 플레이 GIF입니다.
...
5. NPC 근접 시 상호작용 프롬프트 표시 및 E 키를 상호작용 구현
...
인벤토리 패널
탭 버튼과 슬롯 컨테이너로 구성되며, 탭 선택에 따라 아이템이 카테고리별로 필터링 되어 표시됩니다.
